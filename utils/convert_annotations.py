...
import sys
import argparse
from . import convert_pvoc_xml_to_xyxy_txt
from . import convert_darkflow_json_to_xyxy_txt
...
def convert_prediction_files(predictions_dir: str, out_dir: str):
    ...
    if len(ext_list) == 1:
        if ext_list[0] == '.xml':
            ...
            pr_dir = convert_darkflow_json_to_xyxy_txt.convert_prediction_darkflow_json(predictions_dir=predictions_dir,
                                                                                   out_dir=out_dir)
            return pr_dir
        else:
            logger.error("Converter not implemented for annotation file type {}".format(ext_list[0]))

...
import shutil
import unittest
from utils import convert_pvoc_xml_to_yolo_lxywh_txt as sut
...
class TestConvertPascalVocXmlToText(unittest.TestCase):
    ...
    def test_convert_gt(self):
        ...
        out_dir = sut.convert_ground_truth_xml(ground_truth_dir=self.test_gt_dir, out_dir=self.test_out_dir)
    ...
    def test_convert_pr(self):
        if os.path.exists(self.test_out_dir):
            shutil.rmtree(self.test_out_dir)
        out_dir = sut.convert_prediction_xml(predictions_dir=self.test_pr_dir, out_dir=self.test_out_dir)
        self.assertEqual(self.test_out_dir, out_dir)
